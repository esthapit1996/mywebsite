...
export default function GroupDetail() {
  ...
  const handleAddExpense = async (e) => {
    ...
    try {
      ...
      await api.createExpense(id, expenseAmount, expenseDesc, splitType, splitWith);
  ...
  const getActivityColor = (actionType) => {
    switch (actionType) {
      case 'expense_created': return '#e3f2fd';
      case 'expense_deleted': return '#ffebee';
      case 'settlement': return '#e8f5e9';
      case 'payment': return '#f3e5f5';
      case 'member_added': return '#fff3e0';
      case 'member_removed': return '#fce4ec';
                  ...
                  <div className="expense-info">
                    ...
                    {expensePaymentStatus[expense.id] && 
                      ...
                      expensePaymentStatus[expense.id].totalPaid >= expensePaymentStatus[expense.id].totalOwed - 0.01 && (
                        <div style={{ color: '#155724', fontSize: '0.85rem', marginTop: '4px' }}>✅ Settlements were made</div>
                    )}
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <div className="modal-body">
                ...
                {splitType === 'percentage' && group?.members && (
                  <div className="form-group">
                    ...
                    <div style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginBottom: '10px', padding: '10px', background: '#f8f9fa', borderRadius: '8px' }}>
                      <strong>You are paying {expenseAmount ? `$${expenseAmount}` : 'this expense'}.</strong><br/>
                        ...
                        <div key={member.id} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                          ...
                          {expenseAmount && memberSplits[member.id] && (
                            <span style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>
                              = ${(parseFloat(expenseAmount) * parseFloat(memberSplits[member.id]) / 100).toFixed(2)}
                            </span>
                          )}
                        </div>
                      ))}
              ...
              {selectedExpense.splits && selectedExpense.splits.length > 0 && (
                <div style={{ marginBottom: '16px' }}>
                  ...
                  {selectedExpense.splits.map((split) => (
                    <div key={split.user_id} style={{ display: 'flex', justifyContent: 'space-between', padding: '4px 0', borderBottom: '1px solid #eee' }}>
                ...
                {expensePayments.length === 0 ? (
                  ...
                  <div style={{ maxHeight: '150px', overflowY: 'auto' }}>
                    {expensePayments.map((payment) => (
                      <div key={payment.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px', background: '#f0f9f0', borderRadius: '6px', marginBottom: '6px' }}>
                        <div>
                          <div style={{ fontWeight: '500' }}>
